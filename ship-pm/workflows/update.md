...
```bash
# Check Claude
cat ~/.claude/ship-pm/VERSION 2>/dev/null || \
# Check Codex
cat ~/.codex/ship-pm/VERSION 2>/dev/null || \
# Check Gemini
cat ~/.gemini/ship-pm/VERSION 2>/dev/null || \
# Local check
cat .claude/ship-pm/VERSION 2>/dev/null || echo "0.0.0"
```
...
Run:
```bash
npm show ship-pm version 2>/dev/null
```
...
Wait! Our installer doesn't have CLI flags yet. Just tell the user to follow the `npx -y ship-pm@latest` prompts on the screen, or spawn the bash process interactively. 
